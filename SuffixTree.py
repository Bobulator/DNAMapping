...
class SuffixTree:
    ...
    def add_char(self, c):
        ...
        while self.r > 0:
            ...
            if self.active_node == self.root and self.active_length > 0:
                ...
                self.active_node = self.nodes[self.active_node].get_link() if self.nodes[
                                                                                  self.active_node].get_link() > 0 else self.root
    ...
    def print_gv_internal_nodes(self, x, stree):
        if x != self.root and len(self.nodes[x].get_edges()) > 0:
            stree.append("\tnode" + str(x) + " [label=\"" + str(
                x) + "\",style=filled,fillcolor=lightgrey,shape=circle,width=.07,height=.07]" + "\n")
        for key, child in self.nodes[x].get_edges().iteritems():
            self.print_gv_internal_nodes(child, stree)

    def print_gv_suffix_links(self, x, stree):
        if self.nodes[x].get_link() > 0:
            stree.append("\tnode" + str(x) + " -> node" + str(
                self.nodes[x].get_link()) + " [label=\"\",weight=1,style=dotted]" + "\n")
        for key, child in self.nodes[x].get_edges().iteritems():
    ...
    def print_gv_edges(self, x, stree):
        ...
        for key, child in self.nodes[x].get_edges().iteritems():
            ...
            stree.append("\tnode" + str(x) + " -> node" + str(child) + " [label=\"" + self.edge_string(
                child) + "\t" + start + ", " + end + "\",weight=3]" + "\n")
